...
export class ChallengeService {
  ...
  async create(dto: CreateChallengeDto, file?: Express.Multer.File) {
    ...
    if (user.plan === 'FREE') {
      ...
      if (Number(dto.taxaInscricao) > 0 || Number(dto.valorCaucao) > 0 || !dto.isFree) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const challenge = await tx.challenge.create({
        data: {
          ...
          metaSemanal: parseInt(dto.metaSemanal as any, 10),
          taxaInscricao: user.plan === 'FREE' ? 0.00 : dto.taxaInscricao,
          valorCaucao: user.plan === 'FREE' ? 0.00 : dto.valorCaucao,
          isFree: user.plan === 'FREE' ? true : dto.isFree,
          startDate: start,
          endDate: end,
          fileId: fileId,
        },
  ...
  //       throw new BadRequestException('Este desafio já foi encerrado.');
  //     }

  //     // 2. Busca o infeliz do usuário para validar plano e saldo
  //     const user = await tx.user.findUnique({ where: { id: userId } });
  //     if (!user) {
  //       throw new BadRequestException('Usuário não encontrado.');
  //     }

  //     // 3. TRAVA DO PLANO FREE (Protegida dentro da transação contra cliques duplos)
  //     if (user.plan === 'FREE') {
  ...
  //     // 4. Evita duplicidade de inscrição
  ...
  //     const saldoUsuario = Number(user.walletBalance);

  //     if (saldoUsuario < custoTotal) {
  //       throw new BadRequestException(
  //         `Saldo insuficiente. Você precisa de R$ ${custoTotal.toFixed(2)} para entrar (Inscrição: R$ ${taxa.toFixed(2)} + Caução Semanal: R$ ${caucao.toFixed(2)}).`
  //       );
  //     }

  //     // 6. Deduz o saldo da carteira do usuário (se houver custo)
  //     if (custoTotal > 0) {
  //       await tx.user.update({
  //         where: { id: userId },
  //         data: { walletBalance: saldoUsuario - custoTotal }
  //       });
  //     }

  //     // 7. Transfere a taxa de inscrição direto para o criador do desafio (Admin)
  //     if (taxa > 0) {
  //       const creator = await tx.user.findUnique({ where: { id: challenge.creatorId } });
  //       if (creator) {
  //         await tx.user.update({
  //           where: { id: challenge.creatorId },
  //           data: { walletBalance: Number(creator.walletBalance) + taxa }
  //         });
  //       }
  //     }

  //     // 8. Matrícula o participante retendo a caução dele
  ...
  //     // 9. Atualiza o caixa (Treasury) do grupo se houver caução retida
  //     if (caucao > 0) {
  //       const novoTotalEscrowed = Number(challenge.treasury?.totalEscrowed || 0) + caucao;
        
  //       await tx.challengeTreasury.upsert({
  //         where: { challengeId },
  //         // Se o cofre já existir, atualiza o saldo somando a nova caução
  //         update: { 
  //           totalEscrowed: novoTotalEscrowed 
  //         },
  //         // Se o cofre NÃO existir por qualquer motivo, cria ele do zero com o saldo inicial
  //         create: { 
  //           challengeId, 
  //           totalEscrowed: caucao 
  //         }
  //       });
  //     }

  //     // 10. Log de Auditoria para proteção jurídica
  //     await tx.auditLog.create({
  //       data: {
  //         userId,
  //         action: 'JOIN_CHALLENGE',
  //         description: `Usuário ${user.name} entrou no desafio ${challenge.title}. Plano: ${user.plan}. Taxa de inscrição (R$ ${taxa.toFixed(2)}) repassada ao admin. Caução (R$ ${caucao.toFixed(2)}) retida no cofre.`
  //       }
  //     });

  //     return {
  //       message: 'Inscrição realizada com sucesso! Rumo ao Shape! 🏆',
  //       participant
  //     };
  //   });
  // }
